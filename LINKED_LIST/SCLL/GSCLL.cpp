...
template <class T>
void SCLL<T>::InsertAtPos(T value, int pos)
{
...
template <class T>
void SCLL<T>::DeleteAtPos(int pos)
{
    if(pos < 1 || pos > Count)
    {
        cout << "Invalid position!" << endl;
...
template <class T>
void SCLL<T>::Display()        // To display the LL
{
    struct node<T>* temp = Head;
...
template <class T>
int SCLL<T>::CountNode()        // To count number of nodes in LL
{
    return Count;
}
